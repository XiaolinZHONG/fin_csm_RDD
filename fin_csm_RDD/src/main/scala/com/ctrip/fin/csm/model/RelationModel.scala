...
/**
  ...
  */
class RelationModel extends SCMModel("relation") {
  ...
  def predict(data: DataFrame): DataFrame = {
    ...
    val score_1 = predict_1_temp.withColumn("score_relation", predict_1_temp("prediction_1") * 500 + 350).select("score_relation")
    //-------------------------------------------自此可以直接只返回RF----------------------------------------------------//
  ...
  override def processData(data: DataFrame, isUsedForTraining: Boolean): DataFrame = {
    if (isUsedForTraining == true) {
      data.select("uid_flag", "com_passenger_count", "com_idno_count",
        "com_mobile_count", "ord_success_order_cmobile_count")
    } else {
      data.select("com_passenger_count", "com_idno_count",
        "com_mobile_count", "ord_success_order_cmobile_count")
    }
  }
}
